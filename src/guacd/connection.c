...
static int __write_all(int fd, char* buffer, int length) {
    ...
    while (remaining_length > 0) {
        ...
        int written;
#ifdef _WIN32
        GUAC_RETRY_EINTR(written, send(fd, buffer, remaining_length, 0));
#else
...
void* guacd_connection_io_thread(void* data) {
    ...
    while (1) {
#ifdef _WIN32
        length = recv(params->fd, buffer, sizeof(buffer), 0);
#else
...
static int guacd_add_user(guacd_proc* proc, guac_parser* parser, guac_socket* socket) {
     ...
     * On Linux, SCM_RIGHTS duplicated the fd into the receiving process,
     * so we can safely close our local copy.
     * On Windows (thread model), threads share the fd space and we only
     * sent the integer value - closing here would destroy the handle the
     * proc thread needs. The proc thread will close it when done. */
    guacd_close_socket(proc_fd);
#endif
...
#ifdef _WIN32
            /* On Windows thread model, wait for the client to fully
             * disconnect. We poll proc->fd_socket which the proc thread
             * closes on exit (via guacd_exec_proc cleanup). Using recv with
             * a blocking read will return 0 when the peer closes. */
            {
                /* Give user thread time to connect before polling */
                Sleep(200);
                /* Block on the proc-side socket - reads return 0 when
                 * the proc thread's cleanup closes its end */
                char dummy;
                while (recv(proc->fd_socket, &dummy, 1, 0) > 0) {
                    /* Discard any data (shouldn't happen normally) */
                }
                /* Additional grace period for proc cleanup */
                Sleep(100);
            }
#else
...
void* guacd_connection_thread(void* data) {
    ...
    if (ssl_context != NULL) {
        ...
        if (socket == NULL) {
            ...
            guacd_close_socket(connected_socket_fd);
            guac_mem_free(params);
            return NULL;
        }
    }
