...
#ifdef _WIN32
    /**
     * On Windows (thread model), both the "parent" (main thread) and the
     * "child" (proc thread) share the same address space, so they cannot
     * both store their local socket end in fd_socket. This extra field
     * holds the proc-thread-side socket (equivalent to what the child
     * process would see as fd_socket after fork on Linux).
     */
    int proc_fd_socket;
#endif
