...
#ifdef _WIN32
        if (!FreeLibrary((HMODULE) client->__plugin_handle))
            guac_client_log(client, GUAC_LOG_ERROR, "Unable to close plugin: error %lu", GetLastError());
#else
...
#ifdef _WIN32
    /* Load client plugin */
    client_plugin_handle = (void*) LoadLibraryA(protocol_lib);
    if (!client_plugin_handle) {
        guac_error = GUAC_STATUS_NOT_FOUND;
        guac_error_message = "Unable to load plugin via LoadLibrary";
        return -1;
    }

    /* Get init function */
    alias.obj = (void*) GetProcAddress((HMODULE) client_plugin_handle, "guac_client_init");

    /* Fail if cannot find guac_client_init */
    if (alias.obj == NULL) {
        guac_error = GUAC_STATUS_INTERNAL_ERROR;
        guac_error_message = "Plugin does not contain guac_client_init";
        FreeLibrary((HMODULE) client_plugin_handle);
        return -1;
    }
#else
    /* Load client plugin */
