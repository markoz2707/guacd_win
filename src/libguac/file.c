...
#include <direct.h>
#include <process.h>
/* POSIX permission flags not available on MSVC */
#ifndef S_IRWXU
#define S_IRWXU 0700
#endif
#ifndef S_IRGRP
...
#endif
/* openat() is not available on Windows - emulate it */
static int guac_openat_win32(int dirfd, const char* filename, int flags, mode_t mode) {
    (void)dirfd; /* dirfd is not actually used in any meaningful way on Windows */
    /* On Windows, just open the file by name (path was already opened for dir check) */
    return _open(filename, flags, mode);
}
#define openat(dirfd, path, flags, mode) guac_openat_win32(dirfd, path, flags, mode)
#else
#include <sys/types.h>
#include <unistd.h>
#endif
...
#ifdef _WIN32
    /* File locking not supported on Windows via fcntl */
    (void)fd; (void)read_lock;
    return 0;
#else
