...
char* guac_generate_id(char prefix) {
    ...
    /* Generate UUID using Windows RPC */
    UUID uuid;
    RPC_CSTR uuid_str;
    ...
    if (UuidToStringA(&uuid, &uuid_str) != RPC_S_OK) {
        ...
        guac_error_message = "Conversion of UUID to string failed";
        return NULL;
    }
    ...
    /* Copy UUID string, converting to lowercase */
    for (int i = 0; i < GUAC_UUID_LEN; i++)
        identifier[i] = tolower((unsigned char) uuid_str[i]);
