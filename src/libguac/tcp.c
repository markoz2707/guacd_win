...
int guac_tcp_connect(const char* hostname, const char* port, const int timeout) {
    ...
    for (current_address = addresses; current_address != NULL; current_address = current_address->ai_next) {
...
#ifdef _WIN32
        /* Set socket to non-blocking using ioctlsocket */
        u_long nonblock_mode = 1;
        if (ioctlsocket(fd, FIONBIO, &nonblock_mode) != 0) {
            guac_error = GUAC_STATUS_INVALID_ARGUMENT;
        ...
        if ((retval = connect(fd, current_address->ai_addr, current_address->ai_addrlen)) < 0) {
            ...
            if (errno == EINPROGRESS || errno == EINTR) {
...
#ifdef _WIN32
                retval = select(0, NULL, &fdset, NULL, &tv);
#else
                ...
                if (retval > 0) {
                    ...
                    retval = getsockopt(fd, SOL_SOCKET, SO_ERROR, (char*) &so_error, &so_error_len);
        ...
        if (retval > 0) {
            ...
            u_long block_mode = 0;
            if (ioctlsocket(fd, FIONBIO, &block_mode) != 0) {
                guac_error = GUAC_STATUS_INVALID_ARGUMENT;
