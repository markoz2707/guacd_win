...
#include <windows.h>
/**
 * Windows replacement for clock_gettime(CLOCK_REALTIME, ...) which is not
 * available on Windows. Uses GetSystemTimeAsFileTime to obtain the current
 * time and converts it to a struct timespec.
 *
 * @param ts
 *     Pointer to the timespec structure to populate with the current time.
 */
static void clock_gettime_realtime(struct timespec* ts) {
    FILETIME ft;
    ULARGE_INTEGER li;
    ...
    ts->tv_sec = (time_t)(li.QuadPart / 10000000);
    ts->tv_nsec = (long)((li.QuadPart % 10000000) * 100);
}
#endif
...
#ifdef _WIN32
    clock_gettime_realtime(&next_flush);
#else
...
#ifdef _WIN32
    clock_gettime_realtime(&audio_buffer->next_flush);
#else
