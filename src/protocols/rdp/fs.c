...
/* POSIX file permission macros not defined on Windows */
#ifndef S_IRWXU
#define S_IRWXU 0700
#endif
#ifndef S_IRUSR
...
/* errno values not defined on MSVC */
#ifndef ENOTSUP
#define ENOTSUP 129
#endif
#ifndef EISDIR
...
/* Map POSIX mkdir to Windows _mkdir (ignoring mode) */
#define mkdir(path, mode) _mkdir(path)

/* Map ftruncate to _chsize_s */
#define ftruncate(fd, length) _chsize_s(fd, length)

/* S_ISDIR may not be defined on MSVC */
#ifndef S_ISDIR
#define S_ISDIR(m) (((m) & S_IFMT) == S_IFDIR)
#endif

/**
 * Simple fnmatch replacement for Windows.
 * Supports only '*' and '?' wildcards, no character classes.
 */
static int guac_fnmatch(const char* pattern, const char* string) {
    while (*pattern) {
        if (*pattern == '*') {
            pattern++;
            if (*pattern == '\0')
                return 0;
            while (*string) {
                if (guac_fnmatch(pattern, string) == 0)
                    return 0;
                string++;
            }
            return 1;
...
const char* guac_rdp_fs_read_dir(guac_rdp_fs* fs, int file_id) {
    ...
    if (file->dir == NULL) {
#ifdef _WIN32
        /* fdopendir not available on Windows; use opendir with path */
        file->dir = opendir(file->real_path);
#else
...
#ifdef _WIN32
    return guac_fnmatch(pattern, filename) != 0;
#else
...
int guac_rdp_fs_get_info(guac_rdp_fs* fs, guac_rdp_fs_info* info) {
    ...
    if (!GetDiskFreeSpaceExA(fs->drive_path, &free_bytes, &total_bytes,
                             &total_free_bytes)) {
        errno = EIO;
        return guac_rdp_fs_get_errorcode(errno);
    }

    /* Use 4096 as a synthetic block size */
    info->block_size = 4096;
    info->blocks_total = (int)(total_bytes.QuadPart / 4096);
    info->blocks_available = (int)(total_free_bytes.QuadPart / 4096);
    return 0;
#else
    /* Read FS information */
