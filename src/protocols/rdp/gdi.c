...
BOOL guac_rdp_gdi_end_paint(rdpContext* context) {
    ...
    /* AGGRESSIVE FIX: mark the ENTIRE display surface as dirty whenever
     * anything changes. This fixes "orphan pixel" artifacts caused by
     * FreeRDP under-reporting invalid rects during window drag / scroll
     * operations. Cost: every frame transmits the full screen delta
     * rather than just changed rects - perfectly fine on localhost. */
    guac_rect_extend(&current_context->dirty, &current_context->bounds);
