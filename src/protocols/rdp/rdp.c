...
static BOOL rdp_freerdp_load_channels(freerdp* instance) {
    ...
    guac_client_log(client, GUAC_LOG_INFO, "[trace] load_channels: entered");
...
static BOOL rdp_freerdp_pre_connect(freerdp* instance) {
    ...
    rdpContext* context = GUAC_RDP_CONTEXT(instance);
    guac_client* client = ((rdp_freerdp_context*) context)->client;
    guac_client_log(client, GUAC_LOG_INFO, "[trace] pre_connect: entered");

    rdpGraphics* graphics = context->graphics;
    guac_client_log(client, GUAC_LOG_INFO, "[trace] pre_connect: graphics=%p", graphics);

    guac_rdp_client* rdp_client = (guac_rdp_client*) client->data;
    guac_rdp_settings* settings = rdp_client->settings;

    /* Push desired settings to FreeRDP */
    guac_client_log(client, GUAC_LOG_INFO, "[trace] pre_connect: pushing settings");
    guac_rdp_push_settings(client, settings, instance);
    guac_client_log(client, GUAC_LOG_INFO, "[trace] pre_connect: settings pushed");

    /* Init FreeRDP add-in provider */
    freerdp_register_addin_provider(freerdp_channels_load_static_addin_entry, 0);
    guac_client_log(client, GUAC_LOG_INFO, "[trace] pre_connect: addin provider registered");

    /* Init FreeRDP internal GDI implementation */
    if (!gdi_init(instance, guac_rdp_get_native_pixel_format(FALSE))) {
        guac_client_log(client, GUAC_LOG_ERROR, "[trace] pre_connect: gdi_init FAILED");
        return FALSE;
    }
    guac_client_log(client, GUAC_LOG_INFO, "[trace] pre_connect: gdi_init done");
...
static int guac_rdp_handle_connection(guac_client* client) {
    ...
    /* Init client */
    guac_client_log(client, GUAC_LOG_INFO, "[trace] calling freerdp_new()");
    freerdp* rdp_inst = freerdp_new();
    guac_client_log(client, GUAC_LOG_INFO, "[trace] freerdp_new() returned %p", rdp_inst);
    ...
    /* FreeRDP 3.24 expects these callbacks set (NULL may cause crashes) */
    rdp_inst->PostConnect = NULL;  /* Optional, freerdp checks NULL */
    rdp_inst->PostDisconnect = NULL;
    rdp_inst->LogonErrorInfo = NULL;
    ...
    guac_client_log(client, GUAC_LOG_INFO, "[trace] ContextSize=%zu, calling freerdp_context_new()",
        (size_t)rdp_inst->ContextSize);

    if (!freerdp_context_new(rdp_inst)) {
        guac_client_log(client, GUAC_LOG_ERROR, "[trace] freerdp_context_new() FAILED");
    ...
    guac_client_log(client, GUAC_LOG_INFO, "[trace] freerdp_context_new() done, context=%p",
        GUAC_RDP_CONTEXT(rdp_inst));

    ((rdp_freerdp_context*) GUAC_RDP_CONTEXT(rdp_inst))->client = client;
    guac_client_log(client, GUAC_LOG_INFO, "[trace] client pointer stored in extended context");

    /* Full settings push happens inside PreConnect callback. */

    /* Load keymap into client */
    rdp_client->keyboard = guac_rdp_keyboard_alloc(client,
            settings->server_layout);

    guac_client_log(client, GUAC_LOG_INFO, "[trace] keymap loaded, setting cursor");

    /* Set default pointer */
    guac_display_set_cursor(rdp_client->display, GUAC_DISPLAY_CURSOR_POINTER);

    guac_client_log(client, GUAC_LOG_INFO, "[trace] cursor set, downgrading lock");
    ...
    guac_client_log(client, GUAC_LOG_INFO, "[trace] calling freerdp_connect() to %s:%d",
        settings->hostname, settings->port);

    /* Connect to RDP server - wrap in SEH to catch native crashes from FreeRDP */
    BOOL freerdp_ok = FALSE;
#ifdef _WIN32
    __try {
        freerdp_ok = freerdp_connect(rdp_inst);
    }
    __except (EXCEPTION_EXECUTE_HANDLER) {
        DWORD code = GetExceptionCode();
        guac_client_log(client, GUAC_LOG_ERROR,
            "[trace] SEH EXCEPTION in freerdp_connect(): 0x%08lX", (unsigned long)code);
        freerdp_ok = FALSE;
    }
#else
    freerdp_ok = freerdp_connect(rdp_inst);
    ...
    if (!freerdp_ok) {
        guac_client_log(client, GUAC_LOG_ERROR, "[trace] freerdp_connect() FAILED");
        guac_rdp_client_abort(client, rdp_inst);
        goto fail;
    }

    guac_client_log(client, GUAC_LOG_INFO, "[trace] freerdp_connect() succeeded");
...
fail:
    guac_client_log(client, GUAC_LOG_ERROR, "[trace] entering fail: path of guac_rdp_handle_connection");
    guac_rwlock_release_lock(&(rdp_client->lock));
    return 1;
